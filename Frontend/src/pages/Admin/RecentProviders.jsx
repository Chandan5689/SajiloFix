...
import { MdCheckCircle, MdCancel, MdArrowForward, MdBusiness } from 'react-icons/md';
...
export default function RecentProviders() {
  ...
  const fetchRecentProviders = async () => {
    try {
      setLoading(true);
      const response = await api.get('/admin/recent-users/?limit=5&user_type=offer');
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
      <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-linear-to-br from-green-50 to-emerald-50">
        <h2 className="text-lg font-bold text-gray-900">Recent Providers</h2>
        <Link to="/admin/providers" className="flex items-center gap-1.5 text-green-600 hover:text-green-700 font-semibold text-sm transition-colors">
      ...
      {loading ? (
        ...
        <div className="p-8 text-center">
          <MdBusiness className="text-gray-300 text-5xl mx-auto mb-3" />
          ...
          {providers.map((provider) => (
            <div key={provider.id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 bg-linear-to-br from-green-500 to-green-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm shrink-0">
                  {provider.first_name?.charAt(0)?.toUpperCase() || provider.email.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{provider.first_name || provider.email.split('@')[0]}</p>
